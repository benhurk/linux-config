...
if [ $VARIANT == "dark" ] || [ $VARIANT == "light" ]; then
    ...
    nvim_theme=$(cat $themedir/config | grep "nvim-theme" | awk -F '=' '{print $2}')
    sed -i "s/vim.cmd.colorscheme('.*')/vim.cmd.colorscheme('$nvim_theme')/" $HOME/.config/nvim/lua/config/colorscheme.lua
else
